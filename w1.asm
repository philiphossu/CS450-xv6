...
int main(int argc, char **argv) {
   ...
   6:	83 ec 10             	sub    $0x10,%esp
	inodeTBWalker();
   9:	e8 dc 02 00 00       	call   2ea <inodeTBWalker>
	directoryWalker(".");
   e:	c7 04 24 16 07 00 00 	movl   $0x716,(%esp)
  15:	e8 e0 02 00 00       	call   2fa <directoryWalker>
	//deleteIData(10);
	//inodeTBWalker();
	exit();
  1a:	e8 23 02 00 00       	call   242 <exit>
  1f:	90                   	nop
...
strcmp(const char *p, const char *q)
{
  ...
  5d:	84 c0                	test   %al,%al
  5f:	74 2d                	je     8e <strcmp+0x3e>
  61:	0f b6 19             	movzbl (%ecx),%ebx
  64:	38 d8                	cmp    %bl,%al
  66:	74 0e                	je     76 <strcmp+0x26>
  68:	eb 2b                	jmp    95 <strcmp+0x45>
  6a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  70:	38 c8                	cmp    %cl,%al
  72:	75 15                	jne    89 <strcmp+0x39>
    p++, q++;
  74:	89 d9                	mov    %ebx,%ecx
...
{
  while(*p && *p == *q)
  79:	0f b6 02             	movzbl (%edx),%eax
    p++, q++;
  7c:	8d 59 01             	lea    0x1(%ecx),%ebx
}
...
{
  while(*p && *p == *q)
  7f:	0f b6 49 01          	movzbl 0x1(%ecx),%ecx
  83:	84 c0                	test   %al,%al
  85:	75 e9                	jne    70 <strcmp+0x20>
  87:	31 c0                	xor    %eax,%eax
    p++, q++;
  return (uchar)*p - (uchar)*q;
  89:	29 c8                	sub    %ecx,%eax
}
  8b:	5b                   	pop    %ebx
  8c:	5d                   	pop    %ebp
  8d:	c3                   	ret    
...
strcmp(const char *p, const char *q)
{
  ...
  93:	eb f4                	jmp    89 <strcmp+0x39>
  95:	0f b6 cb             	movzbl %bl,%ecx
  98:	eb ef                	jmp    89 <strcmp+0x39>
 ...
 165:	3c 0d                	cmp    $0xd,%al
  ...
  for(i=0; i+1 < max; ){
    ...
    if(cc < 1)
      break;
    buf[i++] = c;
 167:	88 44 1a ff          	mov    %al,-0x1(%edx,%ebx,1)
    if(c == '\n' || c == '\r')
 16b:	74 0c                	je     179 <gets+0x49>
 16d:	3c 0a                	cmp    $0xa,%al
 16f:	74 08                	je     179 <gets+0x49>
gets(char *buf, int max)
{
  int i, cc;
  char c;
  ...
  for(i=0; i+1 < max; ){
 171:	8d 5e 01             	lea    0x1(%esi),%ebx
 174:	3b 5d 0c             	cmp    0xc(%ebp),%ebx
 177:	7c c7                	jl     140 <gets+0x10>
      break;
    buf[i++] = c;
    if(c == '\n' || c == '\r')
      break;
 ...
 17c:	c6 04 30 00          	movb   $0x0,(%eax,%esi,1)
  return buf;
}
 180:	83 c4 2c             	add    $0x2c,%esp
 183:	5b                   	pop    %ebx
 184:	5e                   	pop    %esi
 185:	5f                   	pop    %edi
 186:	5d                   	pop    %ebp
...
stat(char *n, struct stat *st)
{
 ...
 1a6:	e8 d7 00 00 00       	call   282 <open>
  if(fd < 0)
...
stat(char *n, struct stat *st)
{
 ...
 1c3:	89 c6                	mov    %eax,%esi
  close(fd);
 1c5:	e8 a0 00 00 00       	call   26a <close>
  return r;
...
atoi(const char *s)
{
 ...
 1ef:	b8 00 00 00 00       	mov    $0x0,%eax
  while('0' <= *s && *s <= '9')
 1f4:	77 17                	ja     20d <atoi+0x2d>
 1f6:	66 90                	xchg   %ax,%ax
    n = n*10 + *s++ - '0';
 1f8:	83 c1 01             	add    $0x1,%ecx
 1fb:	8d 04 80             	lea    (%eax,%eax,4),%eax
...
memmove(void *vdst, void *vsrc, int n)
{
 ...
 219:	53                   	push   %ebx
 21a:	8b 5d 10             	mov    0x10(%ebp),%ebx
 21d:	8b 75 0c             	mov    0xc(%ebp),%esi
  char *dst, *src;
 ...
 222:	7e 12                	jle    236 <memmove+0x26>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 349:	0f b6 92 1f 07 00 00 	movzbl 0x71f(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 38a:	e8 d3 fe ff ff       	call   262 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 3f6:	e8 67 fe ff ff       	call   262 <write>
...
putc(int fd, char c)
{
 ...
 466:	e8 f7 fd ff ff       	call   262 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 483:	e8 da fd ff ff       	call   262 <write>
  int c, i, state;
  uint *ap;
 ...
 4df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 4e1:	b8 18 07 00 00       	mov    $0x718,%eax
...
putc(int fd, char c)
{
 ...
 510:	e8 4d fd ff ff       	call   262 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
 ...
 540:	e8 1d fd ff ff       	call   262 <write>
...
putc(int fd, char c)
{
 ...
 590:	e8 cd fc ff ff       	call   262 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 69a:	e8 2b fc ff ff       	call   2ca <sbrk>
  if(p == (char*)-1)

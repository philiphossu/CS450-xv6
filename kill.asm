...
main(int argc, char **argv)
{
  ...
  if(argc < 2){
    printf(2, "usage: kill pid...\n");
  3f:	c7 44 24 04 66 07 00 	movl   $0x766,0x4(%esp)
  46:	00 
...
putc(int fd, char c)
{
 ...
 4b6:	e8 e7 fd ff ff       	call   2a2 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 4d3:	e8 ca fd ff ff       	call   2a2 <write>
  int c, i, state;
  uint *ap;
...
free(void *ap)
{
 ...
 5f1:	a1 fc 09 00 00       	mov    0x9fc,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 637:	89 10                	mov    %edx,(%eax)
  freep = p;
 639:	a3 fc 09 00 00       	mov    %eax,0x9fc
}
 63e:	5b                   	pop    %ebx
 63f:	5e                   	pop    %esi
 640:	5f                   	pop    %edi
 ...
 65f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 662:	a3 fc 09 00 00       	mov    %eax,0x9fc
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 689:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 68c:	8b 1d fc 09 00 00    	mov    0x9fc,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 6c9:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 6cb:	3b 15 fc 09 00 00    	cmp    0x9fc,%edx
...
morecore(uint nu)
{
 ...
 6ea:	e8 1b fc ff ff       	call   30a <sbrk>
  if(p == (char*)-1)
 ...
 6fd:	e8 ee fe ff ff       	call   5f0 <free>
  return freep;
 702:	8b 15 fc 09 00 00    	mov    0x9fc,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 724:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 727:	89 15 fc 09 00 00    	mov    %edx,0x9fc
      return (void*)(p + 1);
 ...
 73c:	eb e9                	jmp    727 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 73e:	c7 05 fc 09 00 00 00 	movl   $0xa00,0x9fc
 745:	0a 00 00 
    base.s.size = 0;
 748:	ba 00 0a 00 00       	mov    $0xa00,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 74d:	c7 05 00 0a 00 00 00 	movl   $0xa00,0xa00
 754:	0a 00 00 
    base.s.size = 0;
 757:	c7 05 04 0a 00 00 00 	movl   $0x0,0xa04
 75e:	00 00 00 

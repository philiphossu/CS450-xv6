...
int main(int argc, char **argv) {
   ...
   7:	83 ec 10             	sub    $0x10,%esp
	...
	if(compareWalkers()){
   a:	e8 e3 03 00 00       	call   3f2 <compareWalkers>
   f:	85 c0                	test   %eax,%eax
  11:	0f 85 c1 00 00 00    	jne    d8 <main+0xd8>
		// If compareWalkers finds no differences, notify user
		printf(1,"No Differences Detected\n");
	}
	// Modify the contents of the FS
	mkdir("foo");
  17:	c7 04 24 31 08 00 00 	movl   $0x831,(%esp)
  1e:	e8 77 03 00 00       	call   39a <mkdir>
	printf(1, "----> Created directory foo inside of root\n");
  23:	c7 44 24 04 68 08 00 	movl   $0x868,0x4(%esp)
  2a:	00 
  ...
  3e:	00 
  3f:	c7 04 24 35 08 00 00 	movl   $0x835,(%esp)
  46:	e8 27 03 00 00       	call   372 <open>
	if(fd >= 0){
  ...
  51:	c7 44 24 04 94 08 00 	movl   $0x894,0x4(%esp)
  58:	00 
  ...
  6c:	00 
  6d:	c7 44 24 04 42 08 00 	movl   $0x842,0x4(%esp)
  74:	00 
  75:	89 1c 24             	mov    %ebx,(%esp)
  78:	e8 d5 02 00 00       	call   352 <write>
	close(fd);
  7d:	89 1c 24             	mov    %ebx,(%esp)
  80:	e8 d5 02 00 00       	call   35a <close>
	// Call directoryWalker to show that it can work with a provided filepath
	directoryWalker("foo");
  85:	c7 04 24 31 08 00 00 	movl   $0x831,(%esp)
  8c:	e8 59 03 00 00       	call   3ea <directoryWalker>
	// Corrupt inode 24 corresponding to the foo directory
	printf(1, "\n!! Corrupting inode 24 (foo directory) !! \n");
  91:	c7 44 24 04 bc 08 00 	movl   $0x8bc,0x4(%esp)
  98:	00 
  99:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  a0:	e8 0b 04 00 00       	call   4b0 <printf>
	deleteIData(24);
  a5:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
  ac:	e8 31 03 00 00       	call   3e2 <deleteIData>
	// Compare the walkers again, we expect differences
	if(compareWalkers()){
  b1:	e8 3c 03 00 00       	call   3f2 <compareWalkers>
  b6:	85 c0                	test   %eax,%eax
  b8:	75 37                	jne    f1 <main+0xf1>
		// If compareWalkers finds no differences, notify user
		printf(1,"No Differences Detected\n");
	}
	...
	// Recover the FS
	printf(1, "\n!! Recovering FS !! \n");
  ba:	c7 44 24 04 4f 08 00 	movl   $0x84f,0x4(%esp)
  c1:	00 
  c2:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  c9:	e8 e2 03 00 00       	call   4b0 <printf>
	recoverFS();
  ce:	e8 27 03 00 00       	call   3fa <recoverFS>
...
int main(int argc, char **argv) {
	...
	if(compareWalkers()){
		// If compareWalkers finds no differences, notify user
		printf(1,"No Differences Detected\n");
  d8:	c7 44 24 04 18 08 00 	movl   $0x818,0x4(%esp)
  df:	00 
  e0:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  e7:	e8 c4 03 00 00       	call   4b0 <printf>
  ec:	e9 26 ff ff ff       	jmp    17 <main+0x17>
	printf(1, "\n!! Corrupting inode 24 (foo directory) !! \n");
	deleteIData(24);
	// Compare the walkers again, we expect differences
	if(compareWalkers()){
		// If compareWalkers finds no differences, notify user
		printf(1,"No Differences Detected\n");
  f1:	c7 44 24 04 18 08 00 	movl   $0x818,0x4(%esp)
  f8:	00 
  f9:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
 100:	e8 ab 03 00 00       	call   4b0 <printf>
 105:	eb b3                	jmp    ba <main+0xba>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 449:	0f b6 92 f3 08 00 00 	movzbl 0x8f3(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5df:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5e1:	b8 ec 08 00 00       	mov    $0x8ec,%eax
...
free(void *ap)
{
 ...
 6e7:	89 10                	mov    %edx,(%eax)
  freep = p;
 6e9:	a3 68 0b 00 00       	mov    %eax,0xb68
}
 6ee:	5b                   	pop    %ebx
 6ef:	5e                   	pop    %esi
 6f0:	5f                   	pop    %edi
 ...
 70f:	03 4b fc             	add    -0x4(%ebx),%ecx
    ...
    p->s.ptr = bp;
  freep = p;
 712:	a3 68 0b 00 00       	mov    %eax,0xb68
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 739:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 73c:	8b 1d 68 0b 00 00    	mov    0xb68,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 779:	89 c2                	mov    %eax,%edx
    ...
    if(p == freep)
 77b:	3b 15 68 0b 00 00    	cmp    0xb68,%edx
...
morecore(uint nu)
{
 ...
 7ad:	e8 ee fe ff ff       	call   6a0 <free>
  return freep;
 7b2:	8b 15 68 0b 00 00    	mov    0xb68,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7d4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 7d7:	89 15 68 0b 00 00    	mov    %edx,0xb68
      return (void*)(p + 1);
 ...
 7ec:	eb e9                	jmp    7d7 <malloc+0xa7>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7ee:	c7 05 68 0b 00 00 6c 	movl   $0xb6c,0xb68
 7f5:	0b 00 00 
    base.s.size = 0;
 7f8:	ba 6c 0b 00 00       	mov    $0xb6c,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 7fd:	c7 05 6c 0b 00 00 6c 	movl   $0xb6c,0xb6c
 804:	0b 00 00 
    base.s.size = 0;
 807:	c7 05 70 0b 00 00 00 	movl   $0x0,0xb70
 80e:	00 00 00 

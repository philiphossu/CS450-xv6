...
int main(int argc, char *argv[]) {
  ...
  18:	e8 fd 02 00 00       	call   31a <directoryWalker>
	exit();
  1d:	e8 40 02 00 00       	call   262 <exit>
#include "fcntl.h"
...
int main(int argc, char *argv[]) {
	if(argc < 2){
		directoryWalker(".");
  22:	c7 04 24 46 07 00 00 	movl   $0x746,(%esp)
  29:	e8 ec 02 00 00       	call   31a <directoryWalker>
		exit();
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 379:	0f b6 92 4f 07 00 00 	movzbl 0x74f(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 3ba:	e8 c3 fe ff ff       	call   282 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 496:	e8 e7 fd ff ff       	call   282 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 4b3:	e8 ca fd ff ff       	call   282 <write>
  int c, i, state;
  uint *ap;
 ...
 50f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 511:	b8 48 07 00 00       	mov    $0x748,%eax
...
putc(int fd, char c)
{
 ...
 570:	e8 0d fd ff ff       	call   282 <write>
...
putc(int fd, char c)
{
 ...
 5c0:	e8 bd fc ff ff       	call   282 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 6ca:	e8 1b fc ff ff       	call   2ea <sbrk>
  if(p == (char*)-1)
